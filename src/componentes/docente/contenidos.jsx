...
const Contenidos = () => {
  ...
  useEffect(() => {
    ...
    if (userId) {
      fetch(`http://172.16.19.1:3001/api/usuario/${userId}/temas`)
  ...
  const handleConfirmSubtemaUpload = (subtemaId) => {
    if (subtemaUploadMethod === "upload") {
      ...
      fetch(`http://172.16.19.1:3001/api/upload-subtema-video/${currentTemaId}/${subtemaId}`, {
        method: "POST",
        body: formData,
      })
        .then((response) => response.json())
      ...
      fetch(`http://172.16.19.1:3001/api/upload-subtema-video/${currentTemaId}/${subtemaId}`, {
        ...
        .then((data) => {
          if (data.error) {
            ...
            setTemas(updatedTemas);
            setAlert({ type: "success", message: "Link de video guardado con éxito." });
          }
        })
        .catch((error) => {
          console.error("Error guardando el link de video:", error);
                            ...
                            <div className="control">
                              <div className="select is-fullwidth">
                                <select
                                  ...
                                  onChange={(e) => setSubtemaUploadMethod(e.target.value)}
                                >
                                  <option value="upload">Cargar archivo de video</option>
                                  <option value="link">Ingresar link de video</option>
                                </select>
                          ...
                          <div className="buttons is-centered">
                            <button
                              ...
                              disabled={
                                ...
                                (subtemaUploadMethod === "link" && !subtemaVideoLink[subtema._id]) ||
                                subtemaUploadingVideo
                              }
                            >
                              {subtemaUploadingVideo ? "Subiendo..." : "Subir Video"}
                            </button>
                            <button
                              className="button"
